...
def write_samples(root, octave, chord, inversion, program):
    ...
    for pitch_shift_hz in np.concatenate((np.array([0]), np.random.randint(10, 80, 5))):
        ...
        sample.transform_wav(
            "attack", start_s=0, duration=0.33, pitch_shift_base_freq=freq, volume=volume, pitch_shift_hz=pitch_shift_hz)
        sample.transform_wav(
            "sustain", start_s=0.33, duration=0.33, pitch_shift_base_freq=freq, volume=volume, pitch_shift_hz=pitch_shift_hz)
        sample.transform_wav(
            "decay", start_s=0.66, duration=0.33, pitch_shift_base_freq=freq, volume=volume, pitch_shift_hz=pitch_shift_hz)
    sample.clean()
...
def gen_chord_samples():
    ...
    NUM_SAMPLES = NUM_SAMPLES * 3 * 6  # evelope, pitch_shift
            ...
            for root in ROOTS[:NUM_ROOTS]:
                ...
                for program in GM_PATCHES[:NUM_PATCHES]:
                    for inversion in INVERSIONS:
                        write_samples(root, octave, chord, inversion, program)
